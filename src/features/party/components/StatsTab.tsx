...
function StatRow({ statKey, mage, draft, statPointsLeft, onStage, onUnstage }: StatsTabProps & { statKey: StatKey }) {
    ...
    <div>
      ...
      <div className="flex flex-col gap-1.5">
        {(Object.keys(STAT_META) as StatKey[]).map((key) => (
          <StatRow key={key} statKey={key} mage={mage} draft={draft} statPointsLeft={statPointsLeft} onStage={onStage} onUnstage={onUnstage} />
        ))}
      </div>
    </div>
  );
